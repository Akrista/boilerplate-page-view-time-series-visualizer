...
def draw_bar_plot():
    ...
    # Draw bar plot
    fig = df_bar.plot(kind='bar', legend=True).get_figure()
    plt.xlabel('Years')
...
def draw_box_plot():
    ...
    sns.boxplot(x='month', y='value', data=df_box, ax=axes[1], order=month_order).set(title='Month-wise Box Plot (Seasonality)',xlabel='Month',ylabel='Page Views')
